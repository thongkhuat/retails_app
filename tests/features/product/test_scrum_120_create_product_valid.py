...
'''
  ...
  Steps:
    ...
    2. Validate status code, response schema, SLA, and database entry.
  Expected Result:
    - Status code is 201.
    - Response schema matches defined spec.
    - SLA should <= 3s.
...
def test_scrum_120_create_product_valid():
    ...
    assert response_json["creation_date"] == payload["creation_date"], f"Expected creation_date '{payload['creation_date']}', got '{response_json['creation_date']}'"

    # Validate SLA <= 3s
    assert response_time <= 3, f"Expected response time <= 3s, got {response_time}s"

    # Step 2: Validate product is created in the database by retrieving it
    ...
    assert get_json["id"] == product_id, "Product ID mismatch on retrieval"
    assert get_json["name"] == payload["name"], "Product name mismatch on retrieval"
    assert get_json["type"] == payload["type"], "Product type mismatch on retrieval"
    assert get_json["retail_price"] == payload["retail_price"], "Product retail_price mismatch on retrieval"
    assert get_json["creation_date"] == payload["creation_date"], "Product creation_date mismatch on retrieval"
