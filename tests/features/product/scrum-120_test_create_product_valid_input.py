...
def test_scrum_120_create_product_valid_input():
    ...
    assert float(response_json["retail_price"]) == payload["retail_price"], f"Expected retail_price '{payload['retail_price']}', got '{response_json['retail_price']}'"
    ...
    # Step 2: Verify product is created in the database by retrieving it
    ...
    assert get_json["id"] == product_id, "Product ID mismatch"
    assert get_json["name"] == payload["name"], "Product name mismatch"
    assert get_json["type"] == payload["type"], "Product type mismatch"
    assert float(get_json["retail_price"]) == payload["retail_price"], "Product retail_price mismatch"
    assert get_json["creation_date"] == payload["creation_date"], "Product creation_date mismatch"
