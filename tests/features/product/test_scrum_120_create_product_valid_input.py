...
def test_scrum_120_create_product_valid_input():
    ...
    assert "id" in response_json, "Response JSON must contain 'id'"
    ...
    # (Optional) Validate product is created in the database
    # Here, you could add a GET request to /product/{id} to confirm existence
    # Example:
    # product_id = response_json["id"]
    # get_response = requests.get(f"{BASE_URL}/product/{product_id}", headers=headers)
    # assert get_response.status_code == 200
    # get_json = get_response.json()
    # assert get_json["id"] == product_id
