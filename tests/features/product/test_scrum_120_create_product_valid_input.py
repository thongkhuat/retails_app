...
def test_scrum_120_create_product_valid_input():
    ...
    import uuid
    try:
        uuid_obj = uuid.UUID(response_json["id"], version=4)
        assert str(uuid_obj) == response_json["id"]
    except Exception:
        assert False, f"id '{response_json['id']}' is not a valid UUID4"

    # Validate SLA <= 3s
    assert response_time <= 3, f"Expected response time <= 3s, got {response_time}s"

    # (Optional) Validate product exists in database via GET (if API supports)
    ...
    assert get_json["id"] == response_json["id"], "GET product id does not match created product id"
